...
internal class SpriteDumper
{
	public static void Dump()
	{
		...
		var spritePathBase = Path.Combine(Path.Combine(DataLocation.localApplicationData, "LoadOrder"), "AssetPictures");
	...
	private static void Dump<PrefabType>(string spritePathBase) where PrefabType : PrefabInfo
	{
		...
		for (uint i = 0; i < prefabs; i++)
		{
			try
			{
				...
				assetName = assetName.Substring(0, assetName.LastIndexOf("_Data")).Replace("  ", " ").Trim();

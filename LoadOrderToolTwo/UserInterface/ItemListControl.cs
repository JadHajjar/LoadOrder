...
internal class ItemListControl<T> : SlickStackedListControl<T> where T : IPackage
{
	...
	protected override bool IsItemActionHovered(DrawableItem<T> item, Point location)
	{
		...
		if (item.Item.Workshop)
		{
			...
			if (rects.RedownloadRect.Contains(location))
			{
				setTip(Locale.ReDownloadPackage);
			}
	...
	protected override void OnPaintItem(ItemPaintEventArgs<T> e)
	{
		...
		if (e.Item.Package.Mod is Mod mod)
		{
			...
			if (isIncluded)
			{
				e.Graphics.FillRoundedRectangle(inclEnableRect.Gradient(mod.IsEnabled ? FormDesign.Design.GreenColor : FormDesign.Design.RedColor, 1.5F), inclEnableRect, 4);
			}
			...
			{
				e.Graphics.DrawImage((isIncluded ? Properties.Resources.I_Check : Properties.Resources.I_X).Color(rects.IncludedRect.Contains(CursorLocation) ? FormDesign.Design.ActiveColor : isIncluded ? FormDesign.Design.ActiveForeColor : ForeColor), rects.IncludedRect.CenterR(24, 24));
				e.Graphics.DrawImage((mod.IsEnabled ? Properties.Resources.I_Enabled : Properties.Resources.I_Disabled).Color(rects.EnabledRect.Contains(CursorLocation) ? FormDesign.Design.ActiveColor : isIncluded ? FormDesign.Design.ActiveForeColor : base.ForeColor), rects.EnabledRect.CenterR(24, 24));
			}
		}
		else
		{
			...
			if (isIncluded)
			{
				e.Graphics.FillRoundedRectangle(inclEnableRect.Gradient(FormDesign.Design.GreenColor, 1.5F), inclEnableRect, 4);
			}

			e.Graphics.DrawImage((isIncluded ? Properties.Resources.I_Check : Properties.Resources.I_X).Color(rects.IncludedRect.Contains(CursorLocation) ? FormDesign.Design.ActiveColor : isIncluded ? FormDesign.Design.ActiveForeColor : ForeColor), inclEnableRect.CenterR(24, 24));
		}
		...
		if (report is not null)
		{
			DrawLabel(e, LocaleHelper.GetGlobalText($"CR_{report.Severity}"), Properties.Resources.I_CompatibilityReport_16, (report.Severity switch
			{
				...
				ReportSeverity.Remarks => FormDesign.Design.ButtonColor.MergeColor(FormDesign.Design.RedColor, 85),
				_ => FormDesign.Design.GreenColor.MergeColor(FormDesign.Design.AccentColor, 15)

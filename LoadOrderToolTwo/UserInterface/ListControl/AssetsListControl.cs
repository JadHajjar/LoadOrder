...
internal class AssetsListControl : SlickStackedListControl<Asset>
{
	public AssetsListControl()
	{
		...
		CentralManager.AssetInformationUpdated += (_) => Invalidate();
	}
	...
	protected override IEnumerable<DrawableItem<Asset>> OrderItems(IEnumerable<DrawableItem<Asset>> items)
	{
	...
	protected override void OnPaintItem(ItemPaintEventArgs<Asset> e)
	{
		...
		using var image = new Bitmap(e.Item.IconImage ?? Properties.Resources.I_AssetIcon.Color(FormDesign.Design.IconColor), iconRectangle.Size);
		...
		var timeRect = DrawLabel(e, e.Item.LocalTime.ToLocalTime().ToString("g"), Properties.Resources.I_UpdateTime, FormDesign.Design.AccentColor.MergeColor(FormDesign.Design.BackColor, 75), new Rectangle(textRect.X, e.ClipRectangle.Y, (int)(100 * UI.FontScale), e.ClipRectangle.Height), ContentAlignment.BottomLeft);
	...
	private void GetStatusDescriptors(Asset asset, out string text, out Bitmap? icon, out Color color)
	{
		if (!asset.Workshop && !asset.BuiltIn)
		{
			text = Locale.Local;
		...
		switch (asset.Status)
		{
			...
			case DownloadStatus.NotDownloaded:
				text = Locale.AssetIsNotDownloaded;
				icon = Properties.Resources.I_Question_16;
	...
	private Rectangles GetActionRectangles(Rectangle rectangle, Asset item)
	{
		...
		rects.IconRect = rectangle.Pad(rects.IncludedRect.Right + (4 * Padding.Left)).Align(new Size(iconSize, iconSize), ContentAlignment.MiddleLeft);

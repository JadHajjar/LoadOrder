...
public static class SteamUtil
{
	...
	public static PublishedFileDTO[] HttpResponse2DTOs(string httpResponse)
	{
		...
		dynamic json = JToken.Parse(httpResponse);
	...
	public static async Task<PublishedFileDTO[]> LoadDataAsync(PublishedFileId[] ids)
	{
		...
		{
			var httpResponse2 = await httpResponse.Content.ReadAsStringAsync();
	...
	public static async Task<string> GetPersonaNameAsync(this HttpWrapper httpWrapper, ulong personaID)
	{
		...
		try
		{
			if (httpWrapper.FailureCount >= httpWrapper.MaxConnections)
				return null;
			var url = $@"https://steamcommunity.com/profiles/{personaID}";
	...
	public class PublishedFileDTO
	{
		...
		public static DateTime ToUTCTime(ulong time)
		{
			return kEpoch.AddSeconds(time);
		}
